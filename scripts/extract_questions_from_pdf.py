...
def find_headings(lines):
    ...
    for idx, line in enumerate(lines):
        ...
        if not s:
            continue
        # heading patterns: UNIT/MODULE/CHAPTER/TOPIC/SECTION or uppercase short lines
...
def extract_questions_from_pdf(path, out_csv):
    ...
    agg = df.groupby(['normalized_question','question_text','topic','source_file']).agg(
    ...
    agg = agg[['sample_question_id','normalized_question','question_text','topic','source_file','occurrence_count','first_line']]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description="Extract questions and topics from a PDF into CSV")
    parser.add_argument('pdf', help='Path to PDF file')
    parser.add_argument('--out', default='data/training_dataset.csv', help='Output CSV path')
    args = parser.parse_args()
    extract_questions_from_pdf(args.pdf, args.out)
